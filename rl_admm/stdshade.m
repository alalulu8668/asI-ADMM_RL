function [lineOut, fillOut] = stdshade(amatrix,alpha,acolor,F,lines,smth)
...
if exist('lines','var')==0 || isempty(lines)
    lines='-'; 
end
if ne(size(F,1),1)
    F=F';
...
hold on;
lineOut = plot(F,amean,lines,'color',acolor,'linewidth',2.0); %% change color or linewidth to adjust mean line
if check
    hold off;
end
end
