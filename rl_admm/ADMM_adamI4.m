% ADAM based. use global mu(exponential average)
function [theta_new, z_new, lambda_new, mu_new] = ADMM_adamI4(numAgent,...
    agents,phi_s, syspar, alg, idx, alpha,z_old,mu_old)
% stochastic admm
...
eta0 = eta;
target = 0.1;
dist =norm(mu_old-gd_batch);
% if eta0^2 * dist <= target
%     eta = eta0;
% else
%     eta = target / dist;
% end  
