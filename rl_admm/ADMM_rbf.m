function [theta_new, z_new, lambda_new] = ADMM_rbf(numAgent,...
...
theta_new = 1/(rho_x+tau)*(gd_batch + lambda_old + tau*theta_old + rho_x*z_old);

% lambda_new = lambda_old + gamma*rho*(-theta_new + z_old);
lambda_new = lambda_old + gamma*rho_x*(-theta_new + z_old);

z_new = z_old + 1/numAgent*((theta_new - 1/rho_z * lambda_new) - (theta_old - 1/rho_z*lambda_old));
