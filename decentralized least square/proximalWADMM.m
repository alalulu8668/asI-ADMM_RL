...
%
function [x,y,z,acc,run_time] = proximalWADMM(H,t,It_num,beta,gamma,T,B,L,v,x_o)
...
%         y_t = inv(beta*eye(L))*(-H{i_k1}'*H{i_k1}*y{i_k1} + H{i_k1}'*t{i_k1} + beta*x + z{i_k1});
        ...
        x = x + (y_t - z_t/beta)*beta/(T*beta + gamma) - (y{i_k1} - z{i_k1}/beta)*beta/(T*beta + gamma);
        y{i_k1} = y_t; 
        z{i_k1} = z_t;
        run_time(it) = toc(t1);
    else
...
%         y_t = inv(beta*eye(L))*(-H{i_k1}'*H{i_k1}*y{i_k1} + H{i_k1}'*t{i_k1} + beta*x + z{i_k1});
        z_t = z{i_k1} + beta*(x - y_t);
        x = x*gamma/(T*beta + gamma) + (x (y_t - z_t/beta) - (y{i_k1} - z{i_k1}/beta))*beta/(T*beta + gamma);
        y{i_k1} = y_t; 
        z{i_k1} = z_t;      
        run_time(it) = run_time(it-1) + toc(t1);
    end
    %%%%%%%%%%%%%
    for i=1:1:T
